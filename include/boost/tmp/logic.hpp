	...
	namespace tmp {
		...
		namespace detail {
			...
			struct dispatch<N, and_<lift_<F>, C>> {
				...
				using f = typename dispatch<1, C>::template f<call_<is_<nothing_>, typename foldey<(
						select_foldey_loop(sizeof...(Ts)))>::template f<andy<false, F>, 0, Ts...>>>;
			};
			...
			struct dispatch<N, and_<F, C>> {
				template <typename... Ts>
				using f = typename dispatch<1, C>::template f<call_<is_<nothing_>,typename foldey<(select_foldey_loop(sizeof...(Ts)))>::template f<
			...
			struct dispatch<N, or_<lift_<F>, C>> {
				template <typename... Ts>
				using f = typename dispatch<1, C>::template f<call_<is_<nothing_,not_<identity_>>, typename foldey<(
						select_foldey_loop(sizeof...(Ts)))>::template f<ory<false, F>, 0, Ts...>>>;
			};
			template <template <typename...> class F, typename C>
			...
			struct dispatch<N, or_<F, C>> {
				template <typename... Ts>
				using f = typename dispatch<1, C>::template f<call_<is_<nothing_,not_<identity_>>, typename foldey<(select_foldey_loop(sizeof...(Ts)))>::template f<
